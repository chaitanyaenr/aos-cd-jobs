...
stage('http_test_scale_test') {
	if (HTTP_TEST) {
		...
		node('CCI && US') {
			...
			def setup_pbench = http_test_properties['SETUP_PBENCH']
			def pbench_server = http_test_properties['PBENCH_SERVER']
			def tooling_inventory =	http_test_properties['TOOLING_INVENTORY']
			...
			def gun	= http_test_properties['GUN']
			...
			println	"TOOLING_INVENTORY: '${tooling_inventory}'"
			...
			try {
				...
				parameters: [	[$class: 'LabelParameterValue',	name: 'node', label: node_label	],
						...
						[$class: 'StringParameterValue', name: 'TOOLING_INVENTORY', value: tooling_inventory ],
						...
						[$class: 'StringParameterValue', name: 'GUN', value: gun ],
